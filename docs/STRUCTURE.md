...
| `app/api/recommend/route.ts` | ✅ | 핵심 API(Edge/서울). 반경조회→모드필터→태그필터→거리순 상위20→3카드+풀. 키는 여기서만. 국내 bbox 밖 좌표 400 (D21) | `POST` |
| `app/api/staticmap/route.ts` | ✅ | 시트 지도 미리보기 프록시 — NCP Static Map을 서버에서 호출해 이미지만 반환(키 서버전용). 키 없으면 501→클라 폴백 (D11). 외부출처 Referer 403 + 국내 bbox 밖 400 (D21) | `GET` |
...
| `distance.ts` | ✅ | 도보분↔미터 변환, 거리 계산 + 방위(미니 로케이터용, D11) + 국내 bbox 판정(D21) | `haversine()`, `walkMinutes()`, `bearingDeg()`, `compass8()`, `isInKorea()` |
