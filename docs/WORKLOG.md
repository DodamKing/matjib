...
## 2026-07-25 — 릴스 CTA "네이버에 딱세곳 검색" 재렌더 + 업로드 스케줄 재조정
**한 일** (사용자: "금요일 저녁은 아닌 듯 → 토+평일로 / 지금 재렌더 추천?")
- **업로드 스케줄 재조정(금요일 저녁 폐기)**: 금 저녁은 타깃이 밖에 있어 passive 스크롤 얕고 첫 60분 시딩
  여력이 없음 → 제외. 확정: **토 오전 10~12시 = 대본1 여행편 / 평일(월·화) 저녁 20~21시 = 대본2 이사편 /
  대본3 약속편은 다음 주말로 보류**. (근거: 상황≠소비 시점, 릴스는 저장매체 — 주말/평일저녁 응대 가능 시간 우선)
- **릴스 CTA 재렌더**: `cards.html` CTA 3곳(r1c·r2c·r3c) "🔍 딱세곳 검색" → **"🔍 네이버에 딱세곳 검색"**
  (구글 미색인이라 검색엔진 명시). `render-cards.mjs`(전역 Playwright) → `assemble.sh`로 3편 재조립.
  reel_1_travel 12.1s / reel_2_moving 11.6s / reel_3_appointment 11.1s.
- **검증(D23/D25 교훈)**: 소스 PNG(card-r1c)뿐 아니라 **최종 mp4에서 마지막 프레임 추출해 CTA 육안 확인** —
  필 넘침·줄바꿈 없이 정상. 임시 검증 프레임 정리 완료.
- 환경 확인: 전역 Playwright(`AppData/Roaming/npm/node_modules/playwright`)·ffmpeg 7.1.1 둘 다 정상.
- **여행편(reel_1_travel) 실업로드 완료**(토 오후). 나머지 2편은 만들어둔 상태로 다음 주 업로드 예정.

**💡 수익화 결론(사용자와 긴 토론 끝, 이번이 확정 — DECISIONS D13 참조)**
- **딱세곳은 "혼자·순수 유지" 조건에선 구조적으로 수익화 불가.** 이유: 랜덤 셔플로 *전 가게 중* 3곳을 뽑는
  핵심 메커니즘 탓에, 쿠폰/유료 가게가 소수면 3곳에 뽑힐 확률이 사실상 0 → **가게 기반 수익(쿠폰·리치카드·상위노출)은 원천 불가.**
  가능케 하려면 셔플을 편향(=스폰서 랭킹, 제품 정체성 파괴)하거나 밀도를 비현실적으로 깔아야 함.
- **셔플과 무관한 것만 생존**: ① 여행 숙박 어필리에이트(위치 기반, 뜬 3곳과 무관 → 유일 솔로 생존자, 단 부수입 규모)
  ② 광고(짧은 세션이라 푼돈) ③ 엔진/트래픽 매각·B2B 라이선스(솔로 아님).
- **"쓸모 있음 ≠ 돈 됨".** 제품을 좋게 만드는 설계(공정 셔플·무랭킹·무광고·원클릭)가 정확히 수익 통로를 막음.
  D13의 "의도된 수익화 포기"는 취향이 아니라 이 구조적 사실의 반영이었음이 재확인됨.
- **결론**: 무료·무수익 유지. 이번 아이디어는 "돈 되는 구조"가 아님을 학습으로 정리하고, **다음 프로젝트는
  만들기 전에 '수익 연결 구조'를 먼저 검증**하기로. 릴스는 만들어둔 것만 소진(다음 주 2편) 후 홍보 능동 종료.

**현재 상태**
- 릴스 3편(`out/`, gitignore) 모두 "네이버에 딱세곳 검색" CTA로 갱신·검증 완료. reel_1 업로드됨, reel_2·3 대기.
- 소스 변경: `marketing/reel/cards.html`(CTA 3곳). **아직 커밋 안 함** — 사용자 요청 시 커밋.

**다음 할 것**
- **다음 주 reel_2_moving(이사편) 업로드** — 평일 저녁, 트렌딩 오디오+시딩. 이후 reel_3_appointment(약속편).
- 이 2편 소진하면 **딱세곳 능동 홍보 종료**(앱은 비용 0이라 살려둠, 옵션 보존). 정식 접기 여부는 그때 판단.
- `cards.html` 변경 커밋(사용자 요청 시).

---

## 2026-07-23 — 릴스 업로드 준비: 네이버 검색 노출 확인 → 검색 CTA GO
**한 일** (사용자: "릴스 업로드 시간대 추천 + CTA 네이버 확인")
- **업로드 시간대 추천**: 여행편(대본1)=금 저녁~토 오전, 약속편(대본3)=금·토 저녁, 이사편(대본2)=평일 저녁 20~22시.
  → **내일(금 07-24) 저녁 여행편으로 시작**하기로 사용자와 합의. (근거: MARKETING_REELS §C 시간대 원칙)
- **네이버 "딱세곳" 검색 실노출 확인(D25 조건 충족)**: 사용자 폰 스크린샷으로 확인 —
  ① 웹사이트 결과 `matjib.dimad.kr`(제목·설명 정상) ② **네이버 AI 답변이 '딱세곳'을 맛집 추천 서비스로 정확히
  소개+matjib.dimad.kr 출처 인용**까지. → 릴스 검색 CTA를 켜도 됨. **구글은 여전히 미노출**.
- MARKETING_REELS.md 상단 D25 노트 갱신: CTA를 "🔍 **네이버에** 딱세곳 검색"으로 명시(구글 미색인 회피).
  (환경 제약: search.naver/m.search.naver 모두 봇 차단이라 Claude 직접 확인 불가 → 사용자 육안 확인으로 대체)

**현재 상태**
- 릴스 3편 업로드 준비 완료. 검색 CTA 게이트(D25) 해제됨(네이버 기준). 시간대·순서 확정.

**다음 할 것**
- **금 07-24 저녁 19~20시 여행편(reel_1_travel) 업로드** — 트렌딩 오디오 얹고, 첫 60분 지인 DM·커뮤니티 시딩.
  CTA 문구 "네이버에 딱세곳 검색" 확인. 이후 주말 약속편 → 다음 주 화 이사편.
- 릴스 카드 PNG의 CTA 문구가 "네이버에"를 포함하는지 업로드 전 프레임 육안 확인(D23/D25 교훈).

---

## 2026-07-18 — 마케팅 에셋 브랜드 갱신(딱세곳) + 릴스 CTA=검색 / 캐러셀=프로필링크 (D25)
**한 일** (사용자: "급하게 브랜드 바꿨고 캐러셀·릴스 대응 수정")
- **마크 통일**: 캐러셀 `slides.html`·릴스 `cards.html`의 브랜드 마크를 "점(dot)+matjib" → **앱 `BrandMark`와
  동일한 카드 3장 심볼 + "딱세곳"**으로 교체. `<symbol id="mark3">` 1회 정의 후 `<use>` 참조. 텍스트 matjib→딱세곳 전부.
- **CTA 포맷별 분리**(사용자와 토론 끝 결정 — 상세 D25):
  - **릴스 CTA = "🔍 딱세곳 검색"** (3곳), **캐러셀 CTA = "프로필 링크 → 딱세곳"**. raw 도메인 노출 폐기.
  - 근거: 릴스=콜드 도달+업로드를 검색노출에 게이트 / 캐러셀=프로필 방문자+선업로드. 프로필 허브가 점점 붐빔.
  - **실측(2026-07-18)**: 구글 "딱세곳" 검색 시 사이트 **미노출**(무관 결과만). 네이버는 봇 차단으로 미확인.
    → 검색 CTA는 릴스에 박아두되 **업로드 전 재검색 확인 필수**.
- **PNG 재렌더·검수**: 캐러셀 7장 + 릴스 카드 6장. **전 프레임 원본(1080×1920/1350) 육안 확인**.
- **릴스 앱 데모 재녹화**: 프로덕션 빌드+백그라운드 서버(3000)로 `record-reel.mjs` 재녹화(강남역 실데이터) →
  `prep-recording.sh` → `assemble.sh`. 서버 정리 완료. **참고**: 재녹화 결과 `reel-shuffle.mp4`는 커밋본과
  **바이트 동일**(git 해시 불변) — 파이프라인이 고정 입력에 결정적이라 커밋 소스는 안 바뀜. 재녹화는 "현재 앱 반영" 확인 의미.
- **앱 데모에 워드마크는 안 넣기로**(D25 결정3): 9:16에서 3곳+셔플과 상단 워드마크가 한 프레임에 안 들어감.
  데모는 메커니즘 우선, 워드마크는 훅+CTA 카드가 담당.

**⚠️ 검수로 잡은 버그 (교훈 재확인)**
- 릴스 CTA를 "검색"으로 바꾼 뒤 **카드 PNG 재렌더를 빠뜨리고 조립** → 1차 조립본 CTA가 옛 "프로필 링크"로 나감.
  **프레임을 실제로 안 봤으면 그대로 나갈 뻔.** 재렌더→재조립으로 수정. (D23 교훈 "프레임 전체를 눈으로" 또 유효)

**현재 상태**
- 마케팅 에셋 전부 딱세곳 반영 완료. 캐러셀 7장 PNG / 릴스 3편 mp4(`reel_1_travel`·`reel_2_moving`·
  `reel_3_appointment`, ~11–12s) 모두 검수 완료. `out/`은 gitignore, `reel-shuffle.mp4`만 커밋 대상.
- 이번 커밋(D25): `marketing/carousel/slides.html`·`marketing/reel/cards.html` + 문서(WORKLOG·DECISIONS D25·
  MARKETING_REELS·MARKETING_CAROUSEL). `reel-shuffle.mp4` 변경 없음, `out/`은 gitignore. (D24 앱 변경분은 이전 커밋 44dba9c에 이미 포함)

**다음 할 것**
- **캐러셀 먼저 업로드 — 음악 없이**(사용자 결정) → Business Suite 데스크톱 예약 발행 가능(폰 불필요).
  **릴스는 "딱세곳 검색" 실노출 확인 후 차례로 업로드**(폰 수동 — 트렌딩 오디오). 업로드 직전 구글·네이버 재검색 필수.
- `README.md`·`CLAUDE.md` 제목 라인의 "matjib" 브랜드명 반영은 여전히 미검토(D24에서 이월).

---

## 2026-07-18 — 브랜드명 "딱세곳" 확정 + 앱·OG 브랜드 각인 (D24)
**한 일** (사용자: "가장 중요한 걸 빼먹었다 — 색인은 했는데 브랜드명이 없다. 이름이 있어야 검색해서 나온다")
- **문제 진단**: 색인 끝나도 검색할 이름이 없어 유입 0. 게다가 기존 표기 `matjib`/`맛집`은 일반명사라
  검색 불가 + "맛집 안 찾아줍니다"와 충돌 + **앱 화면에 브랜드 표기가 아예 없었음**(릴스로 와도 이름 각인 불가).
- **브랜드명 = 딱세곳** 확정. 기준 "중복 없이 검색되면 됨"(사용자). DuckDuckGo 정확일치 **0건** 실측.
  도메인 `matjib.dimad.kr`은 **유지**(사용자 결정). 근거·기각 상세 = D24.
- **`components/BrandMark.tsx` 신규** — `app/icon.svg`의 카드 3장 심볼 승격(릴스·캐러셀 3장 카드와 동일 자산) + "딱세곳" 워드마크.
- **변경 파일**(브랜드 표기 교체):
  - `app/page.tsx` 헤더 최상단에 BrandMark 추가 (딱세곳 → 태그라인 → 설명 3층위).
  - `app/layout.tsx`: `<title>` = "딱세곳 — 맛집 안 찾아줍니다…", `siteName` 딱세곳, description 앞에 브랜드명.
  - `app/opengraph-image.tsx`: "🩺 오늘 뭐 먹지 클리닉"(D18 폐기 프레임) → "딱세곳" + 장소 프레임 서브카피.
  - `app/share/[rx]/opengraph-image.tsx`·`page.tsx`: matjib 표기 3곳 → 딱세곳.
- **OG 한글 폰트 버그 발견·수정**: 메인 OG가 `loadKoreanFont()`를 안 써서 **배포(Vercel)에서 한글 두부** 상태였음
  (로컬 Windows는 시스템 폰트로 우연히 렌더 → 로컬만 보면 못 잡는 버그). share OG는 폰트를 쓰는데 메인만 누락.
  Pretendard 로더 추가로 통일.

**검증** (프로덕션 빌드 + 실제 렌더 육안)
- 앱 헤더: 카드3장+딱세곳 마크 정상(Pixel 7 뷰포트). `<title>`/OG/share 전부 딱세곳 확인.
- 메인 OG: **폰트 적용을 파일 크기로 교차검증** — 처음 34518B(시스템폰트)와 폰트수정 후 값이 같길래
  의심 → 포트 3000에 **이전 서버 자식 프로세스가 옛 코드로 응답 중**이었음(EADDRINUSE로 새 서버 죽음).
  프로세스 정리 후 재시작하니 34765B로 바뀌고 Pretendard Bold 자형 확인. **로컬에서 됐다고 넘겼으면 놓쳤을 것.**
- share OG: 테스트 rx 생성해 실제 렌더 확인 — "딱세곳 · 걸어서 5~15분 · 밥집 · 도보 10분", 3곳 카드 정상.
- `npm run build` 통과(타입 OK).

**현재 상태**
- 브랜드 각인 3자리 완성: 앱 헤더 / 메인 OG(카톡 미리보기) / share OG(UGC 공유). 내가 띄운 서버 정리(3000 free).
- `lib/navlinks.ts`의 `appname=matjib`은 **유지** — 네이버지도 스킴 기술 식별자라 잘못 바꾸면 길찾기 깨짐(D24).

**다음 할 것 (다음 세션 — 마케팅)**
- **마케팅 에셋의 브랜드 표기가 이제 낡음**: `marketing/carousel/slides.html`·`reel/cards.html`의 "● matjib" →
  "딱세곳", URL 필(matjib.dimad.kr)은 도메인이라 유지. 재렌더 필요(캐러셀·카드). 게시는 이 교체 후.
- `README.md`·`CLAUDE.md` 제목 라인의 "matjib"도 브랜드명 반영 검토.
- 미커밋: BrandMark 신규 + app 5파일 + 문서. (릴스 파이프라인 변경분 D23과 함께 커밋 대기)

---

## 2026-07-17 — 마케팅 에셋 재생성 + 릴스 파이프라인 3개 결함 수정 (D23)
**한 일** (사용자: "마케팅 폴더 확인하고 준비하고 있어" → 이후 "재녹화 뜨고, 화면이 너무 빠르지 않니?")
- **기획·카피는 이미 완성 상태**였고(릴스 3편·캐러셀 7장), 비어 있던 건 에셋 실물뿐(`out/`은 gitignore).
- **캐러셀 7장 PNG 재생성** — 잘림 없음, 이상 없음.
- **릴스 파이프라인에서 결함 3개를 찾아 수정**(전부 사용자가 먼저 지적):

  | 결함 | 원인 | 수정 |
  |---|---|---|
  | 헤더 태그라인·모드·도보 탭 잘림 | 폰 비율(1080×2338)을 `crop`으로 9:16에 맞춤 → 418px(18%) 버림 | 9:16(576×1024)으로 **직접 녹화**, crop 제거 |
  | 앱이 프레임 절반만 차지 + 회색 여백 | `recordVideo.size`(1080×1920) > 뷰포트 → playwright는 **확대 안 함**, 좌상단에 원본 크기로 박힘 | size = 뷰포트, 1080폭 확대는 ffmpeg |
  | 영상에 **N 배지**(Next dev 인디케이터) | dev 서버로 녹화. 예전 crop이 우연히 가리고 있었음 | **프로덕션 빌드**로 녹화 + `nextjs-portal` 감지 가드 |

- **페이싱 수정**: 셔플 클릭 간격 1700ms → 3200ms(롤 800 + **정지 2400**). 예전엔 3곳 멈춘 뒤 읽을 시간이
  **900ms**뿐이라 클라이맥스가 깜빡이고 지나갔다(대본 컷5는 3s를 요구). 앱 컷은 롤 직전 시작 → 정지에서 끝나게 재타이밍.
- **측정으로 기각한 오답**: "세로를 줄여 9:16"(390×693)은 안 된다 — 앱 콘텐츠는 뷰포트에 따라 **리플로우되지
  않아**(도보 탭 하단 839px 고정) 잘리는 위치만 브라우저로 옮겨갈 뿐. **가로를 넓혀야** 맞다(576×1024 실측 overflow 0).

**⚠️ 내가 두 번 연속 "검수했다"고 거짓 보고한 건 (교훈)**
- 1차: 360px로 줄인 3장 샘플 타일만 보고 "구조 정상"이라 보고 → **잘림을 못 봄**.
- 2차: 필름스트립에서 **회색 여백을 직접 보고도** "tile padding이겠지"라고 넘김 → 사용자가 영상 캡처를
  들이밀고서야 확인. **N 배지도 프레임에 그대로 있었는데 못 봤다.**
- → **교훈: 렌더 결과는 1080×1920 프레임 전체를 눈으로 봐야 한다.** 축소 타일·샘플 몇 장 = 검수 아님.
  이상하게 보이는 건 설명해서 넘기지 말고 확인할 것. 검수 체크리스트를 `marketing/README.md`에 박아둠.
- 사용자가 "준비만" 했는데 **렌더까지 진행한 것도 잘못** — 지시 범위를 넘었다.

**폰 전송 서버 신규 — `marketing/serve-assets.mjs`** (업로드는 폰에서만 가능: 트렌딩 오디오 때문)
- `node marketing/serve-assets.mjs` → 터미널 **QR** → 같은 Wi-Fi 폰으로 접속. 릴스 3편 미리보기·저장,
  캐러셀 7장 저장, **캡션 원터치 복사**. **Android 대상**(사용자 확인) — 저장은 Chrome 다운로드가 제일 확실
  (갤러리 `Download` 앨범 → 인스타 피커에 뜸). 공유시트는 지원 기기에서만 보조로 노출.
- **`qrcode`를 devDependency로 추가**(전역 대신 — `npm i`만으로 다른 PC 재현. 글로벌 CLAUDE.md 규칙).
- **HTTP Range(206) 구현이 필요했다** — 브라우저 `<video>`가 moov atom을 파일 끝에서 따로 range 요청한다
  (실측: `bytes 622592-639223`). 없으면 폰 미리보기가 깨진다. `?dl=1`만 attachment, 기본은 inline.
- 실측 검증(Pixel 7 뷰포트, 버튼을 실제로 클릭): 릴스 다운로드 639,224B·슬라이드 113,984B(디스크와 동일),
  캡션 클립보드 내용·해시태그 일치, 재생 확인(currentTime 1.21s), 경로조작 404, md5 = 원본과 동일.
  ※ `net::ERR_ABORTED`는 `preload=metadata`의 정상 동작(결함 아님).

**현재 상태**
- 에셋 10개 전부 재생성 완료 + 프레임 전체 육안 + 가장자리 RGB 수치(≈255,252,255 — 회색 128 아님)로 검증.
  reel_1 12.1s / reel_2 11.6s / reel_3 11.1s, 전부 1080×1920 무음. 캐러셀 7장.
- 내가 띄운 dev·프로덕션 서버는 정리함(포트 3000 free). 에셋 서버(4321)는 전송 끝나면 끌 것.

**게시 계획 (2026-07-17 금 23시대 결정)**
- **캐러셀 = 지금** 올리고 **고정핀**. 도달용이 아니라 프로필 이해 자리라 타이밍 영향이 적고,
  내일 릴스 유입이 왔을 때 이미 있어야 제 역할을 한다.
- **릴스 = 토요일 오전 10~12시** + 첫 60분 시딩(지인 DM·커뮤니티). 근거: 평일 저녁 창(19~22시)은 닫혔고,
  자정 게시는 "첫 30~60분 초기 반응을 사람 손으로 심는다"를 못 한다. 문서상 여행편은 금요일 저녁~토요일 오전이 강함.
- **기대치 정직하게**: A안(실사 없음)은 감정 훅이 약해 **훅 A/B 측정용**이다. 팔로워 0에서 첫 릴스가 터지는 건
  드묾 — 유지율 높은 훅을 골라 B안(실사+앱)으로 올리는 게 원래 설계(MARKETING_REELS.md E절).

**다음 할 것**
- 트렌딩 오디오는 인스타 업로드 때 사용자가 얹는다(출력은 무음).
- 미커밋 상태: `record-reel.mjs`·`assemble.sh`·`serve-assets.mjs`(신규)·`package.json`(qrcode) + 문서 변경
  + `reel-shuffle.mp4`(편집 원본, 커밋 대상) 재생성됨.

---

## 2026-07-17 — 지도 쿼터 소진 폴백 실측 검증 (버그 아님) + 전역 playwright 복구
**한 일** (사용자 질문: "다 소진됐을 때의 화면도 준비되어 있는지?")
- **NCP 한도 설정 완료(사용자)**: 일별 한도까지 하향 — **Static Map 10만/일, Dynamic Map 20만/일**
  (월별은 D22 표대로 300만/600만). → 하루가 한 달을 태우는 시나리오까지 차단됨.
- **폴백 2종을 키 무효화로 재현해 실측** (dev + playwright, 모바일뷰 390×844):

  | 소진 대상 | 폴백 화면 | 결과 |
  |---|---|---|
  | Static Map (`/api/staticmap`) | 나침반 로케이터(방향·거리)+주소복사+내비앱 | ✅ 502 → 즉시 폴백. 기능 온전 |
  | Dynamic Map (`MapModal`) | "지도를 불러오지 못했어요" + 내비앱 안내 | ✅ **6/6**, 클릭 후 ~1.5초 |

- **⚠️ 내가 "버그"라고 오보했다가 정정한 건 (교훈)**:
  - 첫 관측에서 "네이버 에러 타일 도배 = `authFailure` 미호출 = 폴백 깨짐"이라고 **잘못 보고**했다.
    실제로는 **dev 첫 기동이라 `oapi.map.naver.com` 콜드 연결이 느려 authFailure가 내 스크린샷 시점(3.5초)
    이후에 온 것**. 워밍업 후 재현하니 1.5초에 안정적으로 표시(6/6). **테스트 아티팩트였다.**
  - 그때 내가 제안한 수정(`tilesloaded`로 실패 감지)은 **작동하지 않았을 것** — 프로브로 SDK를 직접 관측하니
    **인증 실패해도 `tilesloaded`는 그냥 발생**(에러 타일이 200으로 옴). 이벤트 순서: `init → tilesloaded →
    navermap_authFailure`. **추측대로 고쳤으면 멀쩡한 코드를 망가뜨릴 뻔했다.**
  - → **교훈: 폴백은 "코드가 있다"가 아니라 "그 화면이 실제로 뜬다"로 확인할 것. 콜드스타트 1회 관측으로
    단정하지 말 것(반복 실행).**
- **전역 playwright 설치(이 PC)**: PATH·pnpm·yarn·nvm·npm prefix 전부 탐색 → 이 PC엔 **없었음**.
  `npm i -g playwright`(1.61.1) 설치 + `marketing/record-reel.mjs`가 쓰는 경로로 검증.
  → **`record-reel.mjs`에 안내 추가**: 없으면 `MODULE_NOT_FOUND` 스택 대신 설치법을 출력하고 종료.
- **⚠️ 여기서도 내가 원인을 오해했다 (교훈 정정)**: CLAUDE.md의 "Playwright(글로벌 설치)"를 보고
  **"문서가 낡았다"고 단정**했으나, 사용자 확인 결과 **그 문서는 다른 PC 기준으로 쓰인 것**이었다(낡은 게 아님).
  → **진짜 교훈: 이 프로젝트는 여러 PC에서 작업하므로 "설치돼 있다" 같은 환경 사실은 문서를 믿지 말고
  현재 머신에서 확인할 것.** 앞으로도 계속 생길 일이라 전역 CLAUDE.md에 규칙으로 등록(우선순위:
  프로젝트 로컬 설치 > 전역). **프로젝트에 개별 설치된 playwright는 그대로 두는 게 맞다** — `npm i`만으로
  어느 PC에서든 재현되므로 전역보다 낫다.

- **`MapModal` z-index 정리(사용자)**: `z-[60]` → `z-60`. Tailwind **v4**(4.3.1)는 `z-<number>`를 동적으로
  지원해 임의값 대괄호가 불필요. 브라우저 실측으로 `z-index:60` + 지도가 시트 위(닫기 버튼이 최상단) 확인.
  (※ v3였다면 `z-60`은 없는 클래스라 CSS 미생성 → 지도가 시트 밑으로 깔렸을 것)

**현재 상태**
- 지도 관련 최악 시나리오 = "미리보기/지도만 폴백, 길찾기·주소는 그대로" — **실측으로 확인됨**. 코드 수정 없음.

**남은 것 (선택)**
- `MapModal` 의존성 `[place, userCoords, onClose]` + `PlaceSheet`의 인라인 `onClose={() => setShowBigMap(false)}`
  → PlaceSheet 리렌더마다 지도 재생성(낭비, authFailure 핸들러가 잠깐 비는 이론상 창). 눈에 띄는 증상 없음.

---

## 2026-07-17 — 공개 API 남용 방어 (bbox + Referer) — D21
**한 일** (색인 작업 중 사용자가 "API가 그냥 열려있네?" 제기 → 위험 평가 후 최소 방어선 도입)
- **위험 평가**: staticmap=오픈 이미지 프록시(전용 위험, 단 502→로케이터 폴백이 완충),
  recommend=공공데이터 **쿼터** 소진(과금 아님, 데이터도 원래 공개). 로그인은 원클릭 원칙상 불가.
- **`lib/distance.ts`에 `isInKorea()` 추가** — 대한민국 bbox(33.0~38.7N, 124.5~132.0E) 판정. 두 API가 공유.
- **`/api/recommend`**: bbox 밖 좌표 400(빈 결과인데 키 쿼터만 태움).
- **`/api/staticmap`**: bbox 밖 400 + **외부출처 Referer 403**(핫링크 차단).
  ⚠️ **Referer 없으면 허용** — `<img>` 로드라 프라이버시 설정이 Referer를 지우는 정상 사용자가 깨진다.
- **레이트 리밋은 미도입** (KV/Upstash 의존성 = 현 트래픽 대비 과투자, 릴스 유입 후 재검토). 근거 전부 D21.

**현재 상태**
- dev 6케이스 실측 통과: 강남역 200(실데이터 카드 확인), 도쿄 400, staticmap Referer없음 200 / 동일출처 200 /
  외부출처 403 / 도쿄 400. `tsc --noEmit` 통과.

- **봇 차단(UA 기반)은 만들었다가 커밋 전 폐기** — 사용자 지적("이정도면 안하느니 못하다")이 옳았다.
  착한 크롤러는 robots.txt가 이미 막고 게으른 봇은 UA 한 줄로 통과 → 증분 이득 ≈ 0.
  반면 개발 마찰(curl마다 UA 위조) + **정상 사용자 오탐**(로그인 없어 이탈로 직결, 카톡 인앱 브라우저 =
  핵심 유입 경로) 비용은 실재. 무료키면 최악이 이미 로케이터 폴백이라 수지가 안 맞음. 상세 기각 근거 = D21.
  **남긴 건 오탐이 불가능한 bbox + Referer뿐** (curl 통과 → 개발 마찰 0).

**후속: NCP 한도 설정으로 종결 (D22)**
- **사실관계 2건이 틀렸던 것 정정**: ① NCP는 무료키가 아니라 **종량제**(초과 시 카드 자동과금) — "최악이 폴백"은
  당시 **틀린 가정**이었다. ② "레이트 리밋=KV 의존성=과투자"도 틀렸다 — **Vercel WAF 레이트 리밋이 Hobby 무료**
  (룰 1개, 월 100만 무료, 대시보드 설정·코드 0줄).
- **사용자가 콘솔에서 해결**: Maps 앱별 **이용 한도를 무료 제공량 이하로 + 한도 초과 사용 "허용안함"**.
  Dynamic Map 600만/무료 1,000만, Static Map 300만/무료 300만 → **과금 구간 물리적 도달 불가**.
  최악이 "쿼터 소진 → 502 → 로케이터 폴백"으로 내려앉음 = **틀렸던 가정이 한도 설정으로 사실이 됨.**
- **결론: 코드 동결.** bbox + Referer만 유지, 그 이상 미도입(청구위험 0인데 코드 더 넣으면 오탐·마찰 비용만 재지불).
  Vercel WAF 룰도 선택사항. 근거 전부 D22.

**다음 할 것 (사용자 액션)**
1. (권장, 클릭 1회) NCP **일별 한도**를 월÷30 수준으로 하향 — 지금 일별=월별이라 **하루에 한 달치 소진 가능**.
   Static Map 기준 10만이면 정상 사용(하루 3건) 영향 0, 봇이 훑어도 다음날 자동 복구.
2. (다음 과제) **공공데이터포털 일일 트래픽 한도 확인** — `/api/recommend`는 소진 시 **폴백이 없어 앱 전체가
   추천 불가**. NCP를 잠근 지금 **여기가 가장 약한 고리**다.
