...
- **⚠️ 내가 "버그"라고 오보했다가 정정한 건 (교훈)**:
  ...
  - → **교훈: 폴백은 "코드가 있다"가 아니라 "그 화면이 실제로 뜬다"로 확인할 것. 콜드스타트 1회 관측으로
    단정하지 말 것(반복 실행).**
- **전역 playwright 복구**: PATH·pnpm·yarn·nvm·npm prefix 전부 탐색 → **어디에도 없음**(브라우저 캐시만 잔존).
  `npm i -g playwright`(1.61.1) 재설치 + `marketing/record-reel.mjs`가 쓰는 경로로 검증.
  → **`record-reel.mjs`에 안내 추가**: 없으면 `MODULE_NOT_FOUND` 스택 대신 설치법을 출력하고 종료.
  (**메모리에 남기지 않음** — 이 사실은 이미 CLAUDE.md에 있었는데 낡아서 오늘 문제가 됐다. 사본을 늘리는 대신
  **코드가 스스로 말하게** 했다. 문서는 낡지만 실행되는 코드는 안 낡는다.)
